...
async def safe_goto_and_wait(page, url, retries=3):
    for attempt in range(retries):
        try:
            ...
            # Handle OneTrust cookie banner by clicking "Accept all Cookies"
            try:
                # 1) Wait for the Accept button
                await page.wait_for_selector("#onetrust-accept-btn-handler", timeout=15_000)
                # 2) Click it
                await page.click("#onetrust-accept-btn-handler")
                print("Clicked 'Accept all Cookies'")
                # 3) Small pause for the banner to disappear
                await asyncio.sleep(1)
            except TimeoutError:
                print("Cookie banner not present (or 'Accept' button not found).")

            # Now wait for your product container
            await page.wait_for_selector(".css-1ve5dpo", state="attached", timeout=30_000)
