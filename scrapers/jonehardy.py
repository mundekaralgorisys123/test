...
async def safe_goto_and_wait(page, url, retries=3):
    for attempt in range(retries):
        try:
            ...
            await page.wait_for_selector(".main-collection__grid", state="attached", timeout=30000)
    ...
    while (page_count <= max_pages):
        ...
        try:
            async with async_playwright() as p:
                ...
                try:
                    # Wait for either the overlay or the product grid to appear
                    overlay_or_grid = await page.wait_for_selector(
                        ".bx-step, .main-collection__grid", 
                        state="attached", 
                        timeout=15000
                    )
                    
                    # Check if the element has the bx-step class
                    class_name = await overlay_or_grid.get_attribute("class")
                    if class_name and "bx-step" in class_name:
                        log_event("Overlay detected, attempting to close...")
                        try:
                            # Try clicking the decline button
                            decline_button = await page.wait_for_selector(
                                "button[data-click='close']", 
                                timeout=5000
                            )
                            await decline_button.click()
                            log_event("Overlay closed by clicking decline button")
                            await asyncio.sleep(1)  # Small delay after closing overlay
                        except Exception as e:
                            log_event(f"Could not find decline button, trying escape: {str(e)}")
                            await page.keyboard.press("Escape")
                            await asyncio.sleep(1)
                except Exception as e:
                    log_event(f"No overlay found or error handling it: {str(e)}")
                ...
                products = await product_wrapper.query_selector_all("div.product-card") if product_wrapper else []
                ...
                for row_num, product in enumerate(products, start=len(sheet["A"]) + 1):
                    ...
                    try:
                        # Get the first img tag within the product card and extract the src attribute
                        ...
                        # Clean up the image URL by removing the width parameter if needed
                        if image_url and image_url != "N/A":
                            image_url = image_url.split('&width=')[0]
                    except Exception:
                        image_url = "N/A"
