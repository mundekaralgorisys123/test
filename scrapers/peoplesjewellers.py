        ...
        try:
            async with async_playwright() as p:
                ...
                for row_num, product in enumerate(products, start=len(sheet["A"]) + 1):
                   ...
                   # Extract gold type
                    gold_type_match = re.search(r"(Black\s+)?Sterling\s+Silver|Black\s+Rhodium", product_name, re.IGNORECASE)
                    kt = gold_type_match.group() if gold_type_match else "Not found"

                    # Extract diamond weight
                    diamond_weight_match = re.search(r"\d+(?:[-/]\d+)?(?:\s+\d+/\d+)?\s*ct(?:\.|\s)*t\.?w\.?", product_name, re.IGNORECASE)
