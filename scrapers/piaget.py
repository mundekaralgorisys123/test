        ...
        try:
            async with async_playwright() as p:
                ...
                for row_num, product in enumerate(products, start=len(sheet["A"]) + 1):
                    try:
                        # Extract product name - from the product-card-body__title element
                        name_tag = await product.query_selector(".product-card-body__title")
                        product_name = (await name_tag.inner_text()).strip() if name_tag else "N/A"
                    except Exception:
                        product_name = "N/A"

                    try:
                        # Extract description - from the text directly after product-card-body__title
                        description_tag = await product.query_selector(".product-card-body > div:not(.product-card-body__title):not(.product-card__price)")
                    ...
                    image_url = "N/A"
                    try:
                        # Get the first source tag in the picture element for the highest resolution image
                        picture_tag = await product.query_selector(".picture picture")
                        if picture_tag:
                            # Get the first source tag (which typically contains the highest resolution)
                            source_tag = await picture_tag.query_selector("source")
                            if source_tag:
                                srcset = await source_tag.get_attribute("srcset")
                                if srcset:
                                    # Extract the highest resolution URL (first one in srcset)
